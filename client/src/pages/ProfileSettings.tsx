...
import {
  Container, Paper, Typography, TextField, Button, Avatar, Box, Grid,
  Switch, FormControlLabel, Alert, IconButton, Tabs, Tab,
  Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle,
  MenuItem, Select, SelectChangeEvent, FormControl, Chip, Card, CardContent, InputAdornment,
  Slider, List, ListItem, ListItemIcon, ListItemText, Snackbar
} from '@mui/material';
import {
  Edit, Save, PhotoCamera, Person, TrendingUp, Notifications, Security,
  VpnKey, Settings as SettingsIcon, DeleteForever, CloudDownload,
  AddCircle, AccountBalance, NotificationsActive, Autorenew, Block
...
const ProfileSettings: React.FC = () => {
  ...
  const handleNestedSelectChange = (section: string, field: string) => (event: SelectChangeEvent) => {
    const value = event.target.value;
    setFormData(prev => {
      const sectionData = prev[section as keyof typeof prev];
      if (typeof sectionData === 'object' && sectionData !== null) {
  ...
  const handleNotificationToggle = (type: keyof typeof formData.notifications) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSecurityToggle = (type: keyof typeof formData.security) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const checked = event.target.checked;
    setFormData(prev => ({
      ...prev,
      security: {
  ...
  const handleTradingPrefToggle = (type: keyof typeof formData.tradingPreferences) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const checked = event.target.checked;
    setFormData(prev => ({
      ...prev,
      tradingPreferences: {
  ...
  const handlePreferredMarketsChange = (event: SelectChangeEvent<string[]>) => {
                    ...
                    <Grid item xs={12} sm={6}>
                      <FormControl fullWidth disabled={!editMode}>
                        <Select
                          ...
                          onChange={handleInputChange('timezone') as any}
                          displayEmpty
                        >
                    ...
                    <Grid item xs={12} sm={6}>
                      <FormControl fullWidth disabled={!editMode}>
                        <Select
                          ...
                          onChange={handleInputChange('language') as any}
                          displayEmpty
                        >
