import { PrismaClient, ProductType, OrderType, OrderStatus, SubscriptionStatus, PaymentStatus } from '@prisma/client';
...
async function main() {
  ...
  // Create some trading strategies for the demo user
  await prisma.tradingStrategy.create({
    data: {
      name: 'MACD Crossover Strategy',
      description: 'A strategy based on MACD indicator crossovers for trend following.',
      config: {
        timeframe: '1h',
        indicators: {
        ...
        entryConditions: ['price_above_ema200', 'macd_bullish_crossover'],
        exitConditions: ['macd_bearish_crossover', 'take_profit_reached', 'stop_loss_reached'],
        riskManagement: {
          stopLoss: 2.0,
          takeProfit: 4.0,
          maxRiskPerTrade: 1.0,
